...
public final class CertAndKeyGen {
	...
	public static void generate (KeyParam param) throws NoSuchAlgorithmException, InvalidKeyException, Exception {
		...
		X500Name name = new X500Name(param.commonName, param.organizationUnit,
									 param.organizationName, param.localityName,
									 param.stateName, param.country);
		Date start = new Date();
		long len = param.days * 24 * 3600000l;
		Date end = new Date();
		...
		X509Certificate cert = keygen.getSelfCertificate(name,
														 ...
														 ext);
		File out = new File(param.keyPath);
		...
		if (type < 3) {
			...
			if (out.exists())
				ks.load(new FileInputStream(out), param.storePass.toCharArray());
			else
				ks.load(null);
			ks.setKeyEntry(param.certOrAlias, key, param.keyPass.toCharArray(),
						   new X509Certificate[]{cert});
			System.out.println(ks);
			OutputStream os = new FileOutputStream(out);
			ks.store(os, param. storePass.toCharArray());
		} else if (type == 3) {
			OutputStream pk8 = new FileOutputStream(out);
			pk8.write(key.getEncoded());
			pk8.flush();
			BASE64Encoder encoder = new BASE64Encoder();
			OutputStream pem = new FileOutputStream(param.certOrAlias);
			pem.write("-----BEGIN CERTIFICATE-----\n".getBytes());
			encoder.encode(cert.getEncoded(), pem);
			pem.write("\n-----END CERTIFICATE-----\n".getBytes());
			pem.flush();
		}
	}
