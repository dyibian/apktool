	...
	class MyAdapter extends ArrayAdapter<String> implements Filterable {
		...
		public Filter getFilter() {
			Filter filter = new Filter() {
				private void add(ArrayList<String> buf,String[] keywords,String keyword){
					if(keywords==null)
						return;
				...
				protected FilterResults performFiltering(CharSequence constraint) {
					...
					if (ss.length == 2) {
						...
						if (_globalLanguage.isBasePackage(pkg)) {
							add(buf,_globalLanguage.getBasePackage(pkg),keyword);
						}
					} else if (ss.length == 1) {
						if (keyword.charAt(keyword.length() - 1) == '.') {
							...
							add(buf,_globalLanguage.getUserWord(),keyword);
							add(buf,_globalLanguage.getKeywords(),keyword);
							add(buf,_globalLanguage.getNames(),keyword);
