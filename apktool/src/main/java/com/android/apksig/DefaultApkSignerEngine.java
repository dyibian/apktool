...
public class DefaultApkSignerEngine implements ApkSignerEngine {
    ...
    private DefaultApkSignerEngine(
        ...
        mV1SignerConfigs =
                (v1SigningEnabled)
				? new ArrayList<V1SchemeSigner.SignerConfig>(signerConfigs.size()) : Collections.<V1SchemeSigner.SignerConfig>emptyList();
        mV2SignerConfigs =
                (v2SigningEnabled)
				? new ArrayList<V2SchemeSigner.SignerConfig>(signerConfigs.size()) : Collections.<V2SchemeSigner.SignerConfig>emptyList();

        Map<String, Integer> v1SignerNameToSignerIndex =
                (v1SigningEnabled) ? new HashMap<String,Integer>(signerConfigs.size()) : Collections.<String,Integer>emptyMap();
    ...
    public OutputJarSignatureRequest outputJarEntries()
        ...
        List<Integer> apkSigningSchemeIds =
                (mV2SigningEnabled) ? Collections.<Integer>singletonList(2) : Collections.<Integer>emptyList();
